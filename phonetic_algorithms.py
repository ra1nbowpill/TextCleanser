...
class PhoneticAlgorithms:
    ...
    def double_metaphone(self, st):
        ...
        is_slavo_germanic = (st.find('W') > -1 or st.find('K') > -1
                             or st.find('CZ') > -1 or st.find('WITZ') > -1)
        length = len(st)
        first = 2
        ...
        while pos <= last:
            ...
            if ch in vowels:
                ...
                if st[pos + 1] == 'H':
                    ...
                    # 'hugh'
                    elif (pos > (first + 1) and st[pos - 2] in ['B', 'H', 'D']) or (pos > (first + 2) and st[pos - 3] in ['B', 'H', 'D']) or (pos > (first + 3) and st[pos - 4] in ['B', 'H']):
                        nxt = (None, 2)
                    else:
                ...
                # -ges-,-gep-,-gel-, -gie- at beginning
                    ...
                    nxt = ('K', 'J', 2)
                # -ger-,  -gy-
                elif (st[pos + 1:pos + 2] == 'ER' or st[pos + 1] == 'Y') and st[first:first + 6] not in ["DANGER", "RANGER", "MANGER"] and st[pos - 1] not in ['E', 'I'] and st[pos - 1:pos + 2] not in ['RGY', 'OGY']:
                    nxt = ('K', 'J', 2)
                # italian e.g, 'biaggi'
                ...
                elif (pos == last and st[pos - 1] in vowels) or st[pos - 1:pos + 5] in ["EWSKI", "EWSKY", "OWSKI", "OWSKY"] or st[first:first + 3] == 'SCH':
                    nxt = ('', 'F', 1)
...
if __name__ == '__main__':
    ...
    for s in words:
        print("{} -> {}".format(s, pa.double_metaphone(s)))

    names = {
        'maurice': ('MRS', None), 'aubrey': ('APR', None), 'cambrillo': ('KMPRL', 'KMPR'),
